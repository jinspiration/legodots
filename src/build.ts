...
import { BoardData, DOTS, Dot } from "./App";
  ...
  switch (action.type) {
    ...
    case ActionType.EDIT:
      ...
      if (key === _key) {
        ...
        const ir = rotate.indexOf(state.board[place][2]);
        if (rotate.length != 1 && ir != rotate.length - 1) {
          const replaceRotate: Dot = [
            current[0],
            current[1],
            rotate[(ir + 1) % rotate.length],
          ];
          console.log("rotating", place, current[2], ir, replaceRotate);
          console.log(replaceRotate);
          return {
            used,
